...
const RemixUIPanelHeader = (props: RemixPanelProps) => {
  ...
  const PluginHeading = ({ plugin }: {plugin: PluginRecord}) => {
    ...
    return (
      <>
        <section className="swapitHeader px-3 pt-2 pb-0 d-flex flex-row">
  ...
  return (
    <header className="d-flex flex-column">
      <div className="px-3 pt-2 pb-0 d-flex flex-row align-items-center">
        {plugin?.profile?.name === 'filePanel' ? <FilePanelHeading /> : <PluginHeading plugin={plugin} />}
      </div>
    </header>
  )
}
