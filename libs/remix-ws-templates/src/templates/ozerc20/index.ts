...
export default async (opts: any, contractName: string = 'MyToken') => {
  ...
  const filesObj = {
    [`contracts/${contractName}.sol`]: erc20.print({ ...erc20.defaults, upgradeable: opts && opts.upgradeable ? opts.upgradeable : false }),
    // @ts-ignore
  ...
  if (!opts || opts.upgradeable === undefined || !opts.upgradeable) {
    // @ts-ignore
    if (erc20.defaults.mintable) filesObj[`tests/${contractName}_test.sol`] = (await import(`raw-loader!./tests/${contractName}_mintable_test.sol`)).default
    // @ts-ignore
