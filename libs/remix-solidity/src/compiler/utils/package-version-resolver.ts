...
/**
 * PackageVersionResolver centralizes version resolution:
 * Priority:
 *  1) workspace resolutions/overrides (package.json)
 *  2) parent package dependencies (if provided)
 *  3) lock files (yarn.lock / package-lock.json)
 *  4) npm fetch (package.json)
 */
export class PackageVersionResolver {
  ...
  // Expose maps for read-only use by callers
  public getWorkspaceResolutions(): ReadonlyMap<string, string> {
    return this.workspaceResolutions
  }
  public hasWorkspaceResolution(name: string): boolean {
    return this.workspaceResolutions.has(name)
  }
  public getWorkspaceResolution(name: string): string | undefined {
    return this.workspaceResolutions.get(name)
  }
  public hasLockFileVersion(name: string): boolean {
    return this.lockFileVersions.has(name)
  }
  public getLockFileVersion(name: string): string | undefined {
    return this.lockFileVersions.get(name)
  }

  /** Load workspace resolutions/overrides and dependency hints */
  ...
  /** Resolve version using the defined priority */
  public async resolveVersion(
    ...
    // Priority 1: workspace resolutions
    ...
    // Priority 2: parent package deps (if provided)
    ...
    // Priority 4: fetch from npm (package.json)
