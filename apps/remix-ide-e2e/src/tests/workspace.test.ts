...
module.exports = {
  ...
  'Should create ERC721 workspace with files #group1': function (browser: NightwatchBrowser) {
    browser
      ...
      .waitForElementVisible('*[data-id="treeViewLitreeViewItemcontracts"]')
      .isVisible('*[data-id="treeViewDivDraggableItemremix.config.json"]')
      .isVisible('*[data-id="treeViewLitreeViewItemcontracts/MyToken.sol"]')
      .click('*[data-id="treeViewLitreeViewItemcontracts/MyToken.sol"]')
      .getEditorValue((content) => {
        browser.assert.ok(content.indexOf(`contract MyToken is ERC721, ERC721Pausable, Ownable, ERC721Burnable {`) !== -1,
          'Incorrect content')
      })
      .pause(300)
      // .clickLaunchIcon('filePanel')
